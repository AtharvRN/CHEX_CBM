...
def parse_args():
    ...
    parser.add_argument("--annotation_dir", type=str, required=True,
                        help="Directory with ChEX annotations")
...
def load_annotations(
    ...
    found = 0
    for idx in tqdm(range(n_images), desc="Loading annotations"):
        ...
        # First entry is image path
        img_entry = data[0]
        image_paths.append(img_entry.get('img_path', ''))
        
        # Rest are annotations
        for ann in data[1:]:
            label = ann.get('label')
            logit = float(ann.get('logit', 0.0))
            
            if label in concept_to_idx and logit > confidence_threshold:
                # Keep max score per concept
                cidx = concept_to_idx[label]
                concept_matrix[idx, cidx] = max(concept_matrix[idx, cidx], logit)
        
        if concept_matrix[idx].sum() > 0:
            found += 1
...
def filter_concepts_by_frequency(
    ...
    """
    Filter concepts by detection frequency.
    ...
    keep_mask = (freqs >= min_freq) & (freqs <= max_freq)
...
def main():
    ...
    val_ann_dir = args.annotation_dir.replace('train', 'val')
    if os.path.exists(val_ann_dir):
        val_concept_matrix, _ = load_annotations(
            val_ann_dir, n_val, concepts, args.confidence_threshold
        )
    else:
        print("No validation annotations found, using zeros")
    ...
    backbone = get_model(args.backbone, num_classes=1, pretrained=True)
    ...
    # Save concept layer
    torch.save(model.concept_layer.state_dict(), 
               os.path.join(args.output, "concept_layer.pt"))
    
    if args.cbl_finetune_backbone:
        torch.save(model.backbone.state_dict(),
                   os.path.join(args.output, "backbone.pt"))
